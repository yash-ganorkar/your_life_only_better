        ...
        switch (type) {
            ...
            case 'hashtag': {
                ...
                switch (this.props.hashtag) {
                    case 'instagram':
                        return [`yolobe://tag?name=${tag}`, `https://www.yolobe.com/explore/tags/${tag}/`];
                    case 'twitter':
                        return [`yolobe://search?query=%23${tag}`, `https://yolobe.com/hashtag/${tag}`];
                    default:
            ...
            case 'mention': {
                ...
                switch (this.props.mention) {
                    case 'instagram':
                        return [`yolobe://user?username=${mention}`, `https://www.yolobe.com/${mention}/`];
                    case 'twitter':
                        return [`yolobe://user?screen_name=${mention}`, `https://yolobe.com/${mention}`];
                    default:
                        return [match.getMatchedText()];
                }
            }
    ...
    renderLink(text, match, index, textProps) {
        ...
        return (
            <Text
                ...
                onPress={() => this.onPress(match, true)}
...
Autolink.propTypes = {
    ...
    hashtag: PropTypes.oneOf([false, 'instagram', 'twitter', 'yolobe']),
    latlng: PropTypes.bool,
    linkStyle: Text.propTypes.style, // eslint-disable-line react/no-typos
    mention: PropTypes.oneOf([false, 'instagram', 'twitter', 'yolobe']),
